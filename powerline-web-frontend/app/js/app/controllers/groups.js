angular.module('app.controllers').controller('groups',function ($scope, groups, $route, topBar, $location, flurry, homeCtrlParams) {
  $scope.loading = true;
  $scope.items = [];
  ...
  $scope.join = function () {
    if ($scope.joinFrom && $scope.joinFrom.$invalid) {
      ...
      if (group.required_permissions && group.required_permissions.length) {
        ...
        $rootScope.confirm(message, join ,'Permissions', 'OK,Cancel');
      } else {
        join();
      }
    }
